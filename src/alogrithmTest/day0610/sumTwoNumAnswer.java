...
class lcstnAnswer {
    /**
     * 暴力法即我采用的直接循环两次的算法时间复杂度O（n2）
     * 时间复杂度：O(n^2)O(n​2​​ )， 对于每个元素，我们试图通过遍历数组的其余部分来寻找它所对应的目标元素，
     * 这将耗费 O(n)O(n) 的时间。因此时间复杂度为 O(n^2)O(n​2​​ )。空间复杂度：O(1)O(1)。
     */
     ...
     * @return
     */
    public int[] twoSumOfTowHash(int[] nums, int target) {
